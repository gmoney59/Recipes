...
class MainForm(QDialog):
    ...
    def __init__(self, parent=None):
		...
		### self.recipes is an imported RecipeContainer object
        self.recipes = recipes.RecipeContainer(QString("recipeBitch.dat"))#     "recipes222.dat"))
        self.model = QStandardItemModel(self.listView)

        self.setWindowTitle("Recipes (dict)")
        QTimer.singleShot(0, self.initialLoad)
    ...
    def populateTable(self, selectedRecipe=None):
        ...
        for row, recipe in enumerate(self.recipes):
            ...
            if selectedRecipe is not None and selectedRecipe == id(recipe):
                selected = item
            self.tableWidget.setItem(row, recipes.TEXT, item)
            self.tableWidget.setItem(row, recipes.ACCESSIBLE_DESCRIPTION, QTableWidgetItem(recipe.accessible_description))
            self.tableWidget.setItem(row, recipes.ACCESSIBLE_TEXT, QTableWidgetItem(recipe.accessible_text))
    ...
    def addRecipe(self):
        ...
        recipe = recipes.Recipe("Unknown", "Unknown", "Unknown")
        self.recipes.addRecipe(recipe)
        self.populateList()
        #self.populateTree()
    ...
    def tableItemChanged(self, item):
        ...
        if column == recipes.TEXT:
            recipe.text = item.text().trimmed()
        elif column == recipes.ACCESSIBLE_DESCRIPTION:
            recipe.accessible_description = item.text().trimmed()
        elif column == recipes.ACCESSIBLE_TEXT:
